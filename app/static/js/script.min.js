var qry={video:"",container:"",canvas:"",ctx:"",out:"",found:!1,timer:"",loop:function(){qry.captureToCanvas();qry.found?(qry.timer&&clearTimeout(qry.timer),qry.out.className="happy",/(http|https):\/\/(\w+:{0,1}\w*@)?(\S+)(:[0-9]+)?(\/|\/([\w#!:.?+=&%@!\-\/]))?/.test(qry.found)?qry.out.innerHTML="<span>Links are not allowed</span>":document.getElementById("device-id").value=qry.found,qry.canvas.style.height=qry.video.clientHeight+"px",qry.container.removeChild(qry.video),qry.container.appendChild(qry.canvas)):
